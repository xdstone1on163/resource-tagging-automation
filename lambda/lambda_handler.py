...
def main(event, context):
    ...
    for arn in resARNs:
        try:
            ...
            # CloudWatch Logs requires a specific API, not the Resource Groups Tagging API
            if ':log-group:' in arn:
                # Extract log group name from ARN: arn:aws:logs:region:account:log-group:log-group-name
                ...
                try:
                    ...
                    print(f"Verification - Current tags on {log_group_name}: {actual_tags}")

                    # Check if all expected tags are present
                    missing_tags = [key for key in _res_tags if key not in actual_tags or actual_tags[key] != _res_tags[key]]
                    if missing_tags:
                        print(f"WARNING: Some tags were not applied successfully: {missing_tags}")
                    else:
                        print(f"Verification successful - All tags applied correctly")
                except Exception as verify_error:
                    print(f"Warning: Failed to verify tags for {log_group_name}: {verify_error}")
            else:
                ...
                try:
                    ...
                    if response['ResourceTagMappingList']:
                        actual_tags = {tag['Key']: tag['Value'] for tag in response['ResourceTagMappingList'][0].get('Tags', [])}
                        print(f"Verification - Current tags on {arn}: {actual_tags}")
                        ...
                        print(f"Warning: Resource {arn} not found during verification")
                except Exception as verify_error:
